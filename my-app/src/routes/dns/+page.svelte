...
<script>
  ...
  onMount(() => {
    ...
    const domainInput = document.getElementById('destination-input');
    ...
    // ✅ use normal event binding (now DOM is ready)
    ...
    resolveBtn.addEventListener('click', async () => {
      ...
      consoleBox.innerHTML = "⏳ Resolving DNS...";
        ...
        if (res.ok && data.results && data.results.length > 0) {
          consoleBox.innerHTML = data.results.map((result, index) => {
            ...
            if (!result.success) {
              ...
              if (result.type === "MX") {
                output = result.addresses.map(a => `${a.exchange} (priority ${a.priority})`).join("\n");
                ...
                output = result.addresses.map(a => (typeof a === "string" ? a : a.host || String(a)).replace(/^www\./i, "")).join("\n");
                ...
                output = result.addresses.map(a => {
                  switch (a.type) {
                    case 'A':
                    case 'AAAA': return `(${a.type}) ${a.address}`;
                    case 'MX': return `(${a.type}) ${a.exchange} (Prio: ${a.priority})`;
                    case 'NS':
                    case 'CNAME':
                    case 'PTR': return `(${a.type}) ${a.value}`;
                    case 'TXT': return `(${a.type}) "${a.entries.join(' ')}"`;
                    case 'SOA': return `(${a.type}) ${a.nsname}`;
                    default: return `(${a.type}) [object Object]`;
                  }
                }).join("\n");
              } else {
                output = result.addresses.join("\n");
  ...
  <div class="box">
    ...
    <div class="container">
      <div class="left">
        ...
        <input type="text" id="destination-input" placeholder="example.com" />
      ...
      <div class="right">
        <div class="console" id="dns-console">Ready to resolve...</div>
      </div>
    </div>
...
<div class="popup-nwtable hidden" id="nwtable-popup">
  <div class="popup-box">
    ...
    <button class="pop_close" id="nwtable-cancel-btn">CLOSE</button>
  </div>
</div>